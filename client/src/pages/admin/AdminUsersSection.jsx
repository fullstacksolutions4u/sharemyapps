...
export default function AdminUsersSection({ initialTab = 'developers' }) {
  ...
  const devRankMap = (() => {
    const map = {};
    users.forEach(u => { map[u._id] = u.rank || 1; });
    return map;
  })();
  const missingCvCount = counts.missingCv || 0;
  const missingSummaryCount = counts.missingSummary || 0;
  const placeholderCvCount = counts.placeholderCv || 0;
  const updatedCvCount = counts.updatedCv || 0;
  const list = users;
  const paged = users;
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        <div className="flex gap-1 bg-[#F3F0EB] p-1 rounded-xl w-fit shrink-0">
          {[
            { key: 'developers', label: `Developers (${counts.developers || 0})` },
            { key: 'recruiters', label: `Recruiters (${counts.recruiters || 0})` },
          ].map(t => (
        ...
        {search && <p className="text-xs text-[#6B7280]">{totalCount} result{totalCount !== 1 ? 's' : ''} for <span className="font-medium text-[#1A1A1A]">"{search}"</span></p>}
      ...
      {totalPages > 1 && (() => {
        ...
        return (
          <div className="flex flex-col items-center gap-2 pt-2">
            <p className="text-xs text-[#9CA3AF]">Showing {totalCount > 0 ? (page - 1) * PER_PAGE + 1 : 0}–{Math.min(page * PER_PAGE, totalCount)} of {totalCount}</p>
